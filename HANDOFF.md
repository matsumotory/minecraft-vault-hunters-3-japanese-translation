...
- **3.21.7-ja.2は配布・審査承認・まっさらE2E再検証まで完了 (2026-08-17)**: GitHub Release 3.21.7-ja.2 (タグ+jar添付) をAPIで作成し、CurseForgeへも公式Upload APIでファイル追加して審査承認済み。承認後にインスタンス削除からの利用者手順E2Eをもう一度通し、アプリがja.2を自動で配ること、2回起動後にハードコード層 (クリスタルツールチップのレベル/容量/目標/テーマ/レイアウト) が日本語になることを画面で確認した。**残る本人操作はCurseForgeのDescription全文の差し替えだけ** (正はdocs/curseforge-description.md。【】装飾を廃して見出しと箇条書きのmarkdown構造へ書き直した版。編集APIが無いため手作業)
- 豆知識: まっさらプロファイルのGUIが小さいのは、modpack同梱の初期options.txtがguiScale:1を固定しているため (Minecraft自身の既定は0=自動)。故障ではない
