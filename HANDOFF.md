...
- **CurseForgeで公開済み (2026-08-17、初回審査で一発承認)**: https://www.curseforge.com/minecraft/mc-mods/vault-hunters-3-japanese-translation 。公開ページでVaultPatcherのRequired Dependencyが有効なことを確認済みで、アプリからの自動導入が動く状態。掲載物は2026-08-17に本人がすべて最新化し、公開ページで反映を確認した (日本語主体のDescription 約14,400、ギャラリー4枚、SourceのGitHubリンク、ファイルChangelogの修正済み数字)

## 次の作業 (順番どおり)

1. 次版 (3.21.7-ja.2) の候補機能: 初回起動の適用が終わったら、ゲーム内 (タイトル画面のトースト等) で「再起動すると翻訳がすべて有効になります」と利用者へ通知する。1回目の起動のまま遊び続ける利用者をなくすため (2026-08-17の本人の指摘。READMEは再起動を手順に組み込む形で対応済み)
2. modpack更新 (3.21.8以降やRemastered対応) が来たら、翻訳作業側で差分を訳してから、生成スクリプトでこちらへ取り込み、バージョンを上げて配布する
